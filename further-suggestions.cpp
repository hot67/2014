 ...
 */
	...
	BuiltinDefaultCode()	{
        ...
        AxisCamera *camera = &AxisCamera::GetInstance("10.0.67.11"); //Double-check this IP and change it if it is wrong
	...
	void DisabledPeriodic()  {
		...
		{
			m_dsLCD->Printf(DriverStationLCD::kUser_Line4,1,"   DEBUG MODE ENABLED  ");
		}
...
*/
	 ...
	 void TeleopBGrabber()
	{
		...
		if (!m_dunRollin)
			 m_roller->Set(-1);
		else if (m_operator->GetRawAxis(TRIGGERS) > 0.4 || m_driver->GetRawButton(BUTTON_RB)) {
			m_roller->Set(1);
		}
...
/*
	...
	*/	
	...
	{   	
		...
		} else if (m_operator->GetRawButton(BUTTON_LB) && m_operator->GetRawButton(BUTTON_X))
		{
			...
			m_armPID->Enable();
		} else if (m_operator->GetRawButton(BUTTON_LB) && m_operator->GetRawButton(BUTTON_Y))
		{
			m_armPIDFlag = true;
	...
	void ArmReset()
	{
	  ...
	  if ((m_operator->GetRawButton(BUTTON_BACK) && m_operator->GetRawButton(BUTTON_Y)) || AutonDBSteps == 7)
	  {
	    if (m_canResetArm)
	    {
	...
	*/
	void MedRamFire() {
		if (m_driver->GetRawAxis(TRIGGERS) > 0.4 && m_ramCase == -1 && m_medRamCase == -1)
			m_medRamCase = 0;
		switch (m_medRamCase)
		{
		case 0:
