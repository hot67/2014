    ...
    while (_listening)
    {
        ...
        try
        {
            while (Timer::GetFPGATimestamp() < _lastHeartbeatTime + HEARTBEAT_TIMEOUT)
            {
                try 
                {
                	printf("- Reading byte from cheesyvision: ");
                    uint8_t byte;
                    stream->read(&byte, 1);
                    printf("%d\n", byte);
                    ...
                    _lastHeartbeatTime = Timer::GetFPGATimestamp();
                    printf("- LastHeartBeatTime: %f : ", _lastHeartbeatTime);
                    printf("%d\n", Timer::GetFPGATimestamp() < _lastHeartbeatTime + HEARTBEAT_TIMEOUT);
                }
